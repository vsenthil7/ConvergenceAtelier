...
## S7 — Event TYPES: one platform, many event shapes  ✅  ← (product vision, COMPLETE)
...
- [x] **S7.6 Type-aware AI drafts** ✅: the AI agenda-draft, specialised per event type. Instead of one generic running order, each type drafts the plan its organiser actually needs — a hackathon drafts a judging schedule, a webinar a promo timeline, a workshop a session plan, a conference a run-of-show. Reuses the S3 embedding engine (keyless in demo mode); no new model fork. Backend ✅ (256 tests, 100% cov) + frontend ✅ (205 tests, 100% lines, prod tsc clean).
